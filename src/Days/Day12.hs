...
execInstruction (Forwards n) = modify (\f -> f {location = moveInDir (direction f) n $ location f})
execInstruction (Move d n) = modify (\f -> f {location = moveInDir d n $ location f})
